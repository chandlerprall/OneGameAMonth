...
float getElevation( vec2 position ) {
    ...
    position /= 10.0;
    elevation += ( 256.0 / 511.0 ) * snoise( position );

    position *= 4.0;
    elevation += ( 128.0 / 511.0 ) * snoise( position );

    position *= 2.5;
    elevation += ( 64.0 / 511.0 ) * snoise( position );

    position *= 4.0;
    elevation += ( 16.0 / 511.0 ) * snoise( position );

    position *= 8.0;
    elevation += ( 8.0 / 511.0 ) * snoise( position );

    position *= 10.0;
    elevation += ( 4.0 / 511.0 ) * snoise( position );

    position *= 10.0;
    elevation += ( 2.0 / 511.0 ) * snoise( position );

    position *= 10.0;
    elevation += ( 1.0 / 511.0 ) * snoise( position );

    return ( elevation + 1.0 ) * 0.5; // 0.0 - 1.0
}

void main() {
    float elevation = getElevation( vec2( vPosition.x, vPosition.z ) );

    float depth = gl_FragCoord.z / gl_FragCoord.w - elevation * 10.0;
    float depth_color = 1.0 - smoothstep( mNear, mFar, depth );
    gl_FragColor = vec4( vec3( depth_color ), 1.0 );
    return;

    vec4 color = vec4( 0.8, 0.7, 0.5, 1.0 );
    ...
    vec4 grassColor = vec4( 0.0, 1.0, 0.0, 1.0 );
