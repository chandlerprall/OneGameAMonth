define(["exports", "lib/react", "js/views/HopComponent", "js/views/SplashComponent", "js/Assets", "js/Events"], function (exports, _libReact, _jsViewsHopComponent, _jsViewsSplashComponent, _jsAssets, _jsEvents) {
    "use strict";
    ...
    var GameComponent = _React.createClass({
        ...
        initScene: function initScene() {
            this.materials.ground_material = new THREE.MeshLambertMaterial({
                map: _jsAssets.Assets.textures.rock_diffuse
            });

            this.scene = new THREE.Scene();

            var ambient_light = new THREE.AmbientLight(13421772);
            this.scene.add(ambient_light);

            var directional_light = new THREE.DirectionalLight(5592405);
            ...
            directional_light.shadowDarkness = 1;
            ...
            this.player = new THREE.Mesh(_jsAssets.Assets.geometries.monkey, _jsAssets.Assets.materials.monkey);
            ...
            var lava_geometry = new THREE.PlaneBufferGeometry(this.lava_size, this.lava_size),
                lava_material = new THREE.MeshBasicMaterial({ map: _jsAssets.Assets.textures.lava_diffuse, fog: false }),
                lava;
        ...
        spawnGround: function spawnGround() {
        ...
        handleOrient: function handleOrient(e) {
            var rotation = Math.min(Math.max(e.beta, -20), 20);
            this.player_direction = -rotation / 2;
        },

        render: function render() {
            return _React.createElement("div", { id: "GameComponent" });
        },
        ...
        step: function step() {
            ...
            if (last_frame_time) {
                ...
                // Check for death
                this.raycaster.set(new THREE.Vector3(this.player.position.x, 5, this.player.position.z), new THREE.Vector3(0, -1, 0));
                var intersection = this.raycaster.intersectObjects(this.ground, false);
                if (intersection.length === 0) {
                    this.is_dead = true;
                    _jsEvents.Events.emit("changeView", [_jsViewsSplashComponent.SplashComponent]);
                }
